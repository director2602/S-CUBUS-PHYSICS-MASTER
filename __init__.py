...
class GeminiProvider:
    ...
    def __init__(self, model: Optional[str] = None, name: str = "gemini"):
        ...
        self.limiter = limiter_for("gemini:" + self.model, GEMINI_RPM)

    def structured(self, system: str, prompt: str, model_cls: Type[T],
                   max_tokens: int = 12000) -> T:
        def call():
            self.limiter.wait()                      # stay under the free-tier RPM
...
class PhysicsMasterAgent:
    ...
    def __init__(self):
        ...
        if ANTHROPIC_API_KEY:                      # best quality: two different AI vendors
            ...
            self.prov_a = GeminiProvider(GEMINI_MODEL, "gemini-a")
            self.prov_b = GeminiProvider(GEMINI_MODEL_2 or GEMINI_MODEL, "gemini-b")
