...
CLAUDE_MODEL=claude-sonnet-5          # any current Claude model id
GEMINI_MODEL=gemini-2.5-pro           # any Gemini model id your key can use
...
OVERSHOOT = 1.6              # request this multiple of what is still missing
...
class Quota:
    ...
    def __init__(self, request: PaperRequest, blueprint: Blueprint):
        ...
        for k, v in blueprint.recommended_distribution.items():
...
important problem archetypes, a question-type distribution (keys must be from
{request.question_types}, values are counts) and a difficulty distribution
(keys easy/medium/hard, values are counts).
"""
...
class PhysicsMasterAgent:
        ...
        for p in (self.claude, self.gemini):
            try:
                ...
                raise RuntimeError(
                    ...
                    f"Check the API key and model name "
                    f"({'CLAUDE_MODEL' if p.name == 'claude' else 'GEMINI_MODEL'}).") from exc
    ...
    def verify(self, q: Question) -> Tuple[bool, str]:
        ...
        checks = {
            ...
            "not_known_problem": not audit.resembles_known_problem,
        }
    ...
    def generate_paper(self, request: PaperRequest, progress=None) -> List[Question]:
        ...
        for rnd in range(1, MAX_ROUNDS + 1):
            ...
            for i, (qtype, m) in enumerate(missing.items()):
                ...
                provider = (self.claude, self.gemini)[(rnd + i) % 2]
...
"""
        return self.agent.claude.structured(MASTER_SYSTEM, prompt, ErrorAnalysis, max_tokens=3000)
    ...
    def generate(self, original: Question, analysis: ErrorAnalysis,
        ...
        for i in range(attempts):
            provider = (self.agent.claude, self.agent.gemini)[i % 2]
            prompt = f"""
